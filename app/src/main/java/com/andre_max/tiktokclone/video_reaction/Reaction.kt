package com.andre_max.tiktokclone.video_reaction
...
class Reaction(private val remoteUserVideo: RemoteUserVideo, private val likedIcon: ImageView) {
    ...
    fun checkIfVideoWasLiked() {
        val myLikedVideos = firebaseDatabase.getReference(getUserLikedVideosPath()).child(remoteUserVideo.key)
    ...
    fun likeVideo() {
        val myLikedVideos = firebaseDatabase.getReference(getUserLikedVideosPath()).child(remoteUserVideo.key)
        val actualVideoRef = firebaseDatabase.getReference("${getAllVideosPath()}/${remoteUserVideo.key}").child("likes")
        val creatorVideoRef = firebaseDatabase.getReference(getUserPublicVideosPath(remoteUserVideo.creatorUid)).child("${remoteUserVideo.key}/likes")
        ...
        myLikedVideos.setValue(remoteUserVideo.key)
