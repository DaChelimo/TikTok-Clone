...
class MainComment(
    ...
    private fun getComments() {
        val commentsRef = firebaseDatabase.getReference(getCommentsPath(remoteUserVideo.firestoreId))
...
//                adapter.updateAsync(totalCommentsMap.values as MutableList<out Group>) { Timber.d("update completed") }
    ...
    private fun getCommentSize() {
        val totalCommentsRef = firebaseDatabase.getReference(getAllVideosPath()).child(remoteUserVideo.firestoreId).child("totalCommentsSize")
    ...
    fun addComment(commentText: String) {
        val myCommentsRef = firebaseDatabase.getReference(getCommentsPath(remoteUserVideo.firestoreId)).push()
        ...
        addCommentToCommentSize(firebaseDatabase.getReference(getAllVideosPath()).child(remoteUserVideo.firestoreId).child("totalCommentsSize"))
    ...
    fun deleteComment(comment: Comment) {
        val myCommentsRef = firebaseDatabase.getReference(getCommentsPath(remoteUserVideo.firestoreId)).child(comment.commentKey)
        removeCommentToCommentSize(firebaseDatabase.getReference(getAllVideosPath()).child(remoteUserVideo.firestoreId).child("totalCommentsSize"))
    ...
    class EachCommentGroup(private val comment: Comment, private val remoteUserVideo: RemoteUserVideo): Item<GroupieViewHolder>() {
        ...
        private fun likeComment(comment: Comment, likeCommentBtn: ImageView) {
            val commentRef = firebaseDatabase.getReference(getCommentsPath(remoteUserVideo.firestoreId))
...
//            myLikedCommentsRef.addListenerForSingleValueEvent(removeLikeVideoValueEventFun(myLikedCommentsRef))
        ...
        private fun getCommentLikes(commentLikes: TextView, comment: Comment, remoteUserVideo: RemoteUserVideo) {
            val commentRef = firebaseDatabase.getReference(getCommentsPath(remoteUserVideo.firestoreId)).child(comment.commentKey).child("commentLikes")
